...
try {
    console.log("🔹 [PIEEngine] Starting PIE run...");
    console.log("🔹 [PIEEngine] Raw intent:", userIntent);
...
Return only the improved intent, no commentary.
    ...
    console.log("🔹 [PIEEngine] Sending clarify request to OpenAI...");
    ...
    console.log("✅ [PIEEngine] Clarify response:", clarifyRes);

    const refinedIntent = clarifyRes.choices[0].message.content.trim();
    console.log("✅ [PIEEngine] Refined Intent:", refinedIntent);
...
7. Deliver only the final copy-pasteable prompt (no explanation).
    ...
    console.log("🔹 [PIEEngine] Sending architect request to OpenAI...");
    ...
    console.log("✅ [PIEEngine] Architect response:", finalRes);

    const finalPrompt = finalRes.choices[0].message.content.trim();
    console.log("✅ [PIEEngine] Final Prompt:", finalPrompt);
