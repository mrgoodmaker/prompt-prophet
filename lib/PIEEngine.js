...
export async function runPIE(userIntent) {
  ...
  try {
    // --------------------------
    // LAYER 4: First Refinement
    // --------------------------
    const clarifyPrompt = `
You are the PIE Engine (Promptception Intelligence Engine).
Rewrite the user's raw goal as a **clear, high-impact instruction** in their voice.
Remove vague verbs ("help", "tell"), filler words, and ambiguity.
Make it **precise, outcome-focused, actionable**, and **strategic**.

User Input:
"${userIntent}"

Return only the improved intent. Do NOT explain.
    `;

    const firstPass = await client.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: "You are a master at refining vague goals into laser-focused instructions." },
        { role: "user", content: clarifyPrompt }
      ],
      temperature: 0.2
    });

    let refinedIntent = firstPass.choices[0].message.content.trim();
